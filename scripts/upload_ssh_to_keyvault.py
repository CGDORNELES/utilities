...
It is intended for educational and operational use in Azure environments that support Managed Identity.
...
def upload_to_keyvault(vault_name, public_secret_name, private_secret_name, private_key, public_key, client_id=None):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate an SSH key pair and upload it to Azure Key Vault using Managed Identity.")
    ...
    parser.add_argument("--client-id", help="(Optional) Client ID of a User Assigned Managed Identity")
    ...
    upload_to_keyvault(args.vault_name, public_secret, private_secret, private_key, public_key, args.client_id)
